...
- **Undeliverable detection**: Press `V` (or run `:undeliverable` / `:stuck`, or select **Undeliverable** under Dashboards) to list every resource cluster-wide that wants to reach a state and cannot: Pending Pods that will not schedule, Pending PVCs that will not bind, Services with no ready endpoint in their EndpointSlices, Ingresses with no address, and anything held open by a finalizer. The mirror of orphan detection: orphans are what nobody references, undeliverables are what cannot proceed. Every reason is read from a first-class field, a status condition, or an Event on the object, so a row with no such explanation is left out rather than guessed at.
