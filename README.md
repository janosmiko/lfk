...
| `?` | Which-key action panel: hotkeys actionable right now, descriptions colored by category with a color legend at the bottom (scroll: `Ctrl+D`/`Ctrl+U`, close: `?` or `esc`). Also in the YAML, log, and describe viewers, where it lists what that viewer supports in its current state. `F1` opens full help |
