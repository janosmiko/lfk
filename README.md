...
- **Pinned dashboard summaries**: Pin any resource type's status summary (Jobs failed/succeeded, Argo app health/sync, any CRD's phase/conditions rollup) onto the cluster dashboard for a one-page daily health check. Toggle from the action menu (`x`) at the resource types level, or set `pinned_summaries` in config (max 10 per scope; stored per-context or per named union set)
