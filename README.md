...
- **Log Top** — aggregate a resource's logs by parsed attributes (method, host, path, status, or JSON/logfmt keys) showing request counts, REQ/s, share, and errors. Launch from the resource action menu ("Log Top"). In view: `g` group by, `p` profile, `enter` drill down, sort key to re-sort, `esc` back.
