...
- **Blast radius in destructive confirms**: The delete, drain, and scale dialogs state what the action costs before you answer, as a `Removes` row and a `Budget` row — pods removed, ready replicas left, and whether a PodDisruptionBudget is exceeded. Only a drain is actually refused by a budget, since only the eviction API honours one, and the wording distinguishes the two. See [Delete confirm dialog](docs/keybindings.md#delete-confirm-dialog).
- **Dependent count per cascade policy**: The delete confirms add a `Dependents` row that walks `ownerReferences` down from the target and says what the selected policy does to them — `3 pods, 2 replicasets also removed` under Background, `stay in the cluster` under Orphan. `Tab` restates the count in place, deep chains such as Deployment to ReplicaSet to Pod are counted in full, and bulk delete shows one total for the selection. See [Delete confirm dialog](docs/keybindings.md#delete-confirm-dialog).
