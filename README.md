...
- **Merged kubeconfig loading**: `~/.kube/config`, `~/.kube/config.d/*` (recursive, symlinks followed), and `KUBECONFIG_DIR` env var. The `KUBECONFIG` env var is exclusive like kubectl — when set it replaces `~/.kube/config` and the default `config.d/` scan (explicit `--kubeconfig-dir`/`KUBECONFIG_DIR` still merge).
