...
- **Resource templates**: Create resources from 25+ built-in templates plus your own (`a`, `/` to search); includes a Custom Resource template as a starting point. Drop manifests in `~/.config/lfk/templates/` or save one there with `x` -> `T` Export Template
- **Export as template**: `x` -> `T` Export Template strips a live object down to an appliable manifest (no `status`, `uid`, `resourceVersion`, `nodeName`, `clusterIP`, injected token volume, ...) and sends it to the clipboard, a file, or the template list. Secret values are always blanked (keys and `type` kept). Allowed in read-only mode
