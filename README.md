...
- **Export as template**: `x` -> Export Template strips a live object down to an appliable manifest (no `status`, `uid`, `resourceVersion`, `nodeName`, `clusterIP`, injected token volume, ...) and sends it to the clipboard, a file, or the template list. Secret values are always blanked (keys and `type` kept). Allowed in read-only mode
