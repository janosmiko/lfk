...
- **Permission-aware actions**: Pod actions your RBAC refuses are dropped from the action menu, decided by one bulk `SelfSubjectAccessReview` pass per namespace and cached per context. Fails open when the review errors or times out. See [Permission-Aware Actions](docs/usage.md#permission-aware-actions).
