...
func AllKinds() []ProfileKind {
	return []ProfileKind{ProfileTraefikJSON, ProfileJSON, ProfileLogfmt}
}
...
// Traefik wins ties over generic JSON because it is more specific.
func DetectKind(sample []string) ProfileKind {
	order := []ProfileKind{ProfileTraefikJSON, ProfileLogfmt, ProfileJSON}
	best := ProfileJSON
	bestHits := 0
