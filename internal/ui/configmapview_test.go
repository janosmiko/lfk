...
func TestRenderConfigMapEditorTable(t *testing.T) {
	...
	t.Run("selected row keys are present", func(t *testing.T) {
		...
		// handles the visual cue); just assert the data lands in output.
		cm := &model.ConfigMapData{
...
func TestRenderConfigMapEditorOverlay(t *testing.T) {
	t.Run("nil configmap shows error", func(t *testing.T) {
		result := RenderConfigMapEditorOverlay(nil, 0, false, "", 0, "", 0, 0, "", false, nil, false, 0, 0, 100, 40)
	...
	t.Run("normal mode hints removed from overlay body", func(t *testing.T) {
		...
		result := RenderConfigMapEditorOverlay(cm, 0, false, "", 0, "", 0, 0, "", false, nil, false, 0, 0, 100, 40)
	...
	t.Run("editing mode hints removed from overlay body", func(t *testing.T) {
		...
		result := RenderConfigMapEditorOverlay(cm, 0, true, "key1", 4, "value1", 6, 1, "", false, nil, false, 0, 0, 100, 40)
		assert.Contains(t, result, "ConfigMap Editor")
	})
	...
	t.Run("small screen clamps dimensions", func(t *testing.T) {
		...
		result := RenderConfigMapEditorOverlay(cm, 0, false, "", 0, "", 0, 0, "", false, nil, false, 0, 0, 30, 10)
		assert.Contains(t, result, "ConfigMap Editor")
	})
}
