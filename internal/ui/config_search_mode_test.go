...
func TestSearchMode_ResetsOnReload(t *testing.T) {
	for _, next := range []string{"confirm_on_exit: true\n", "search_mode: bogus\n"} {
		t.Run(next, func(t *testing.T) {
			snapshotSearchModeGlobal(t)

			LoadConfig(writeConfigFile(t, "search_mode: fuzzy\n"))
			LoadConfig(writeConfigFile(t, next))
