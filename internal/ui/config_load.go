...
type SecurityIgnorePattern struct {
	...
	// resources within a group, never the whole group — and only matches
	// findings whose resource exposes labels (heuristic-observed pods plus any
	// finding on the same resource). Empty = no label constraint.
