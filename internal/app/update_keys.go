...
func (m Model) handleTabSwitchKey(msg tea.KeyMsg) (tea.Model, tea.Cmd, bool) {
	...
	if m.mode == modeExplorer || m.mode == modeExec || m.yamlView.searchMode || m.logSearchActive || m.helpSearchActive || m.explainSearchActive || m.diffView.searchMode || m.describeSearchActive {
		return m, nil, false
	}
	switch msg.String() {
	case kb.NextTab:
