...
func TestTitleCase(t *testing.T) {
	...
	// Snake-case label keys from sources (Trivy, Falco, etc.) must surface
	// as space-separated words so the rendered column header reads
	// "INSTALLED VERSION" rather than "INSTALLED_VERSION".
	assert.Equal(t, "Installed Version", titleCase("installed_version"))
	assert.Equal(t, "Fixed Version", titleCase("fixed_version"))
	assert.Equal(t, "Host Pid", titleCase("host_pid"))
