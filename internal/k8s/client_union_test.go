...
// prepends a "Cluster" column, and merges/sorts results. Per-context
...
func TestGetResourcesUnion_FanOutStampingAndSort(t *testing.T) {
	...
	for i, want := range expected {
		...
		// Every row must carry a "Cluster" column, and it must sit FIRST so
		// the table renderer shows it as the leftmost extra column.
		require.NotEmpty(t, items[i].Columns)
		assert.Equal(t, "Cluster", items[i].Columns[0].Key, "row %d: Cluster must be the first column", i)
		assert.Equal(t, want.cluster, items[i].Columns[0].Value, "row %d: Cluster value must match ClusterName", i)
	}
}
