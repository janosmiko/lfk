...
func RescaleRightsizing(data *model.Rightsizing, newHeadroom float64) *model.Rightsizing {
	...
	for i, c := range data.Containers {
		nc := c
		nc.CPU = rescaleResourceRec(c.CPU, ratio)
		nc.Mem = rescaleResourceRec(c.Mem, ratio)
		out.Containers[i] = nc
	}
	return &out
}
...
func rescaleResourceRec(r model.ResourceRec, ratio float64) model.ResourceRec {
	out := r
	out.RecommendedRequest = scaleQuantityByRatio(r.RecommendedRequest, ratio)
	out.RecommendedLimit = scaleQuantityByRatio(r.RecommendedLimit, ratio)
	return out
}
...
func scaleQuantityByRatio(q string, ratio float64) string {
	if q == "" || ratio == 1 {
		return q
	}
