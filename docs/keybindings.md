...
| `x` | At resource types level: hide/show the selected resource type via the action menu (saved per cluster context / union set) |
| `H` | Toggle rarely used + hidden resource types (CSI internals, webhooks, APF, leases, advanced core) in the sidebar (rare toggle resets each launch; per-type hides persist) |
