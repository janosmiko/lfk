...
| `startup_resource` | string | *(none)* | Open directly on this resource type's list: a plural/singular/Kind name (e.g. `deploy`, `Deployment`) or `name.group` for a CRD. Overridden by `--resource`. |
...
| `search_mode` | string | `"default"` | Default match type for every search/filter input: `default` (substring, auto-regex, `~` fuzzy, `\` literal), `fuzzy` (plain input matches fuzzy), `regex` (plain input matches regex). `~` and `\` still override per query. |
