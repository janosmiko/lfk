...
| `which_key_enabled` | bool | `true` | Show the goto popup while the `g` prefix is pending, and the action panel while the which-key leader (`?`) is armed (explorer, YAML view, log viewer, describe view). |
