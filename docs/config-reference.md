...
| `icons` | string | `"auto"` | Icon display mode (`auto`/`unicode`/`nerdfont`/`simple`/`emoji`/`none`). Also decides whether the which-key panel draws modifier symbols (`⌃D`) or names (`ctrl+d`). |
