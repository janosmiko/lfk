...
| `security.ignore_patterns` | list | _(empty)_ | Declarative glob-based ignore rules applied at load. Each entry has `cluster`, `source`, `group`, `namespace` (globs; empty = any), an optional `labels` map (label key -> glob value; all entries must match), and an optional `comment`. A finding is hidden when every non-empty field matches. A `labels` entry is resource-scoped (hides matching resources, never the whole group) and only matches findings whose resource exposes labels (heuristic-observed pods and same-pod findings). An all-empty entry is dropped. Read-only (not un-ignorable from the UI); the `i` toggle still reveals them. |
