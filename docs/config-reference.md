...
| `field_manager` | string | `lfk:<os-user>` | Field manager name sent on every write. The API server stores it in `metadata.managedFields`, which the YAML blame view (`b`) reads, so a change shows the person and not only the tool. Set `lfk` to keep your username off the cluster. Cut to 128 characters. |
